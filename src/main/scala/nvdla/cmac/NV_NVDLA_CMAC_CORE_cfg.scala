...
class NV_NVDLA_CMAC_CORE_cfg(implicit val conf: cmacConfiguration) extends RawModule {
...
//             └──┴──┘       └──┴──┘ 
    ...
    val op_done_d1 = RegInit(false.B)
    val cfg_reg_en = RegInit(false.B)
    val cfg_is_wg = RegInit(false.B)
    val cfg_reg_en_d1 = RegInit(false.B)

    val cfg_reg_en_w = (!op_en_d1|op_done_d1) & io.reg2dp_op_en
    val cfg_is_wg_w = false.B//wg is not completed by nvdla yet
