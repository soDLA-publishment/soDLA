...
class NV_NVDLA_RT_cmac_a2cacc(implicit val conf: cmacConfiguration) extends Module {
    ...
    val mac2accu_data_d = retiming(Vec(conf.CMAC_ATOMK_HALF, UInt(conf.CMAC_RESULT_WIDTH.W)), conf.RT_CMAC_A2CACC_LATENCY)
    val mac2accu_pvld_d = retiming(Bool(), conf.RT_CMAC_A2CACC_LATENCY)
    val mac2accu_mask_d = retiming(Vec(conf.CMAC_ATOMK_HALF, Bool()), conf.RT_CMAC_A2CACC_LATENCY) 
    val mac2accu_mode_d = retiming(Bool(), conf.RT_CMAC_A2CACC_LATENCY)
    val mac2accu_pd_d = retiming(UInt(9.W), conf.RT_CMAC_A2CACC_LATENCY)
