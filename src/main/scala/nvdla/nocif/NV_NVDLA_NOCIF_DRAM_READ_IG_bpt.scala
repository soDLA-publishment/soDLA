...
class NV_NVDLA_NOCIF_DRAM_READ_IG_bpt(implicit conf: nvdlaConfig) extends Module {
    val io = IO(new Bundle{
        ...
        //dma2bpt
        val dma2bpt_req_pd = Flipped(DecoupledIO(UInt((conf.NVDLA_MEM_ADDRESS_WIDTH+15).W)))
...
withClock(io.nvdla_core_clk){
    /////////////////// pipe1 /////////////////////
    val pipe_p1 = Module(new NV_NVDLA_IS_pipe(conf.NVDLA_MEM_ADDRESS_WIDTH+15))
    ...
    val pipe_p2 = Module(new NV_NVDLA_IS_pipe(conf.NVDLA_MEM_ADDRESS_WIDTH+15))
    ...
    val in_vld_pd = Fill(79, in_vld) & in_pd

    // PKT_UNPACK_WIRE( dma_read_cmd , in_ , in_vld_pd )
    val in_addr = in_vld_pd(conf.NVDLA_MEM_ADDRESS_WIDTH-1, 0)
    val in_size = in_vld_pd(conf.NVDLA_MEM_ADDRESS_WIDTH+14, conf.NVDLA_MEM_ADDRESS_WIDTH)

    val stt_offset = if(conf.NVDLA_MEMORY_ATOMIC_SIZE == log2Ceil(conf.NVDLA_PRIMARY_MEMIF_WIDTH)) Some(in_addr(conf.NVDLA_MEMORY_ATOMIC_LOG2+1, conf.NVDLA_MEMORY_ATOMIC_LOG2)) 
                      else Some(in_addr(conf.NVDLA_MEMORY_ATOMIC_LOG2+2, conf.NVDLA_MEMORY_ATOMIC_LOG2))

    val size_offset = if(conf.NVDLA_MEMORY_ATOMIC_SIZE == log2Ceil(conf.NVDLA_PRIMARY_MEMIF_WIDTH)) Some(in_size(1, 0)) 
                      else Some(in_size(2, 0))

    val end_offset = stt_offset.get +& size_offset.get

    val is_single_tran = (stt_offset.get + in_size) < (conf.NVDLA_MEM_MASK_BIT*4).asUInt

    val ftran_size = Mux(is_single_tran, size_offset.get, ((conf.NVDLA_MEM_MASK_BIT*4-1).asUInt - stt_offset.get))

    val ftran_num = ftran_size + 1.U

    val ltran_size = Mux(is_single_tran, 0.U(3.W), end_offset)
    val ltran_num = Mux(is_single_tran, 0.U(4.W), (end_offset+1.U))

    val mtran_num = in_size + 1.U - ftran_num - ltran_num
    ...
    out_swizzle := Mux((conf.NVDLA_MEMORY_ATOMIC_LOG2.U === conf.NVDLA_PRIMARY_MEMIF_WIDTH_LOG2.U), false.B, (stt_offset.get(0)===true.B))
    val out_odd = Mux((conf.NVDLA_MEMORY_ATOMIC_LOG2.U === conf.NVDLA_PRIMARY_MEMIF_WIDTH_LOG2.U), false.B, (in_size(0)===true.B))
    ...
    when(bpt2arb_accept){
        ...
        .otherwise{
            if(conf.NVDLA_MEMORY_ATOMIC_LOG2 == conf.NVDLA_PRIMARY_MEMIF_WIDTH_LOG2){
                ...
                out_addr := out_addr +& (8.U << (conf.NVDLA_MEMORY_ATOMIC_LOG2.U - 1.U))
            }
        }
    }
