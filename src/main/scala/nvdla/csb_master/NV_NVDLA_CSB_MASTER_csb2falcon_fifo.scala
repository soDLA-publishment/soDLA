...
class NV_NVDLA_CSB_MASTER_csb2falcon_fifo(implicit val conf: nvdlaConfig)  extends Module {
    ...
    val wr_count = withClockAndReset(wr_clk_wr_mgated, !io.wr_reset_){RegInit("d0".asUInt(2.W))}	// write-side 
    ...
    val wr_adr = withClockAndReset(wr_clk_wr_mgated, !io.wr_reset_){RegInit(0.U)}				// current write address
    ...
    val rd_adr = withClockAndReset(rd_clk_rd_mgated, !io.rd_reset_){RegInit(false.B)}	
    ...
    val ram = Module(new nv_flopram(dep = 2, wid = 34, wr_reg = true))
...
//
    ...
    val rd_count_p =  withClockAndReset(rd_clk_rd_mgated, !io.rd_reset_){RegInit("b0".asUInt(2.W))}
    ...
    val nv_AFIFO_rd_data =  withClockAndReset(rd_clk_rd_mgated, !io.rd_reset_){Reg(UInt(34.W))}
