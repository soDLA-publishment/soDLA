...
//==============
    ...
    val cfg_base_addr = Cat(io.reg2dp_base_addr_high, io.reg2dp_base_addr_low)
    ...
    val cfg_data_mode_per_kernel = io.reg2dp_rdma_data_mode === true.B

//==============
// WIDTH Direction
//==============

    val cfg_proc_int8 = io.reg2dp_proc_precision === false.B
    val cfg_proc_int16 = io.reg2dp_proc_precision === true.B 

    val cfg_mode_wino = io.reg2dp_winograd === true.B
...
//==============
    ...
    val size_of_height = Mux(cfg_mode_wino, 
                            (Cat(0.U(2.W), io.reg2dp_height(12,2))), 
                            io.reg2dp_height)
...
//==============
    ...
    when(cfg_proc_int8){
        size_of_surf := Cat(false.B, io.reg2dp_channel(12,5))
    }.elsewhen(cfg_proc_int16){
        size_of_surf := io.reg2dp_channel(12,4)
    }.otherwise{
        size_of_surf := io.reg2dp_channel(12,4)
    }
...
// Size_Of_Width: As each element is 1B or 2B, the width of cube will be resized accordingly 
    ...
    when(cfg_mode_wino){
        size_of_width := Cat(0.U(3.W), io.reg2dp_width(12,1))
    }.elsewhen(cfg_proc_int8){
        when(cfg_data_use_both){
            when(cfg_data_size_1byte){
                size_of_width := (io.reg2dp_width << 1) + 1.U
            }.otherwise{
                size_of_width := (io.reg2dp_width << 2) + 3.U
            }
        }.otherwise{
            when(cfg_data_size_1byte){
                size_of_width := Cat(0.U(2.W), io.reg2dp_width)
            }.otherwise{
                size_of_width := (io.reg2dp_width << 1) + 1.U
            }
        }
...
// VALID: clamp when when cq is not ready
    ...
    val dma_rd_req_vld_d = cmd_process & io.ig2cq_prdy
...
//==============
    ...
    val int_rd_req_ready_d0 = Wire(Bool())
    int_rd_req_ready := int_rd_req_ready_d0 
