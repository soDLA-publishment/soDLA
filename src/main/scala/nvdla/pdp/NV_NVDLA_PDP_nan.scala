...
// class NV_NVDLA_PDP_nan(implicit val conf: pdpConfiguration) extends Module {
//     val io = IO(new Bundle {
//         // clk
//         val nvdla_core_clk = Input(Clock())
//         //rdma2dp
//         val pdp_rdma2dp_valid = Input(Bool())
//         val pdp_rdma2dp_ready = Output(Bool())
//         val pdp_rdma2dp_pd = Input(UInt(((conf.NVDLA_PDP_BWPE*conf.NVDLA_PDP_THROUGHPUT)+14).W))
//         //preproc
//         val nan_preproc_pvld = Output(Bool())
//         val nan_preproc_prdy = Input(Bool())
//         val nan_preproc_pd = Output(UInt(((conf.NVDLA_PDP_BWPE*conf.NVDLA_PDP_THROUGHPUT)+14).W))
//         //config  
...
//     val din_prdy_d1 = io.nan_preproc_prdy
//     val waiting_for_op_en = RegInit(true.B)

//     io.pdp_rdma2dp_ready := (~din_pvld_d1 | din_prdy_d1) & (~waiting_for_op_en)
//     val load_din = io.pdp_rdma2dp_valid & io.pdp_rdma2dp_ready
...
//     val layer_end = Cat(io.pdp_rdma2dp_pd((conf.NVDLA_PDP_BWPE*conf.NVDLA_PDP_THROUGHPUT)+13), io.pdp_rdma2dp_pd((conf.NVDLA_PDP_BWPE*conf.NVDLA_PDP_THROUGHPUT)+9)).andR & load_din;
...
//     when(load_din){
//         datin_d := io.pdp_rdma2dp_pd
//     }

//     when(io.pdp_rdma2dp_valid & (~waiting_for_op_en)){
//         din_pvld_d1 := true.B
//     }
