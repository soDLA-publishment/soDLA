...
withClock(io.nvdla_core_clk){
    ...
    // DMA Interface
    //==============
    io.dma_rd_cdt_lat_fifo_pop := io.lat_fifo_rd_pd.valid & io.lat_fifo_rd_pd.ready
    ...
    // Context Queue: read
    //==================================================================
    val ig2eg_size = io.cq2eg_pd.bits(14,0)
    val ig2eg_cube_end = io.cq2eg_pd.bits(15)
    ...
    val is_beat_end = is_last_beat & io.lat_fifo_rd_pd.valid & io.lat_fifo_rd_pd.ready
    io.cq2eg_pd.ready := is_beat_end

    when(io.lat_fifo_rd_pd.valid & io.lat_fifo_rd_pd.ready){
        when(is_last_beat){
            beat_count := 0.U
        }.otherwise{
    ...
    u_rdma_unpack.io.inp <> io.lat_fifo_rd_pd
    ...
    u_alu.io.rod_wr_pd.bits     := alu_rod_pd
    u_alu.io.rod_wr_mask        := alu_rod_mask
    u_alu.io.rod_wr_pd.valid    := alu_rod_vld
    alu_rod_rdy                 := u_alu.io.rod_wr_pd.ready

    u_alu.io.roc_wr_pd.bits     := alu_roc_pd
    u_alu.io.roc_wr_pd.valid    := alu_roc_vld
    alu_roc_rdy                 := u_alu.io.roc_wr_pd.ready
    ...
    u_mul.io.rod_wr_pd.bits     := mul_rod_pd
    u_mul.io.rod_wr_mask        := mul_rod_mask
    u_mul.io.rod_wr_pd.valid    := mul_rod_vld
    mul_rod_rdy                 := u_mul.io.rod_wr_pd.ready

    u_mul.io.roc_wr_pd.bits     := mul_roc_pd
    u_mul.io.roc_wr_pd.valid    := mul_roc_vld
    mul_roc_rdy                 := u_mul.io.roc_wr_pd.ready
