...
class NV_NVDLA_SDP_WDMA_DAT_out(implicit val conf: nvdlaConfig) extends Module {
   val io = IO(new Bundle {
        ...
        //out dfifo
        val dfifo_rd_pd = Vec(4, Flipped(DecoupledIO(UInt(conf.AM_DW.W))))
...
withClock(io.nvdla_core_clk){
    ...
    //pop comand data
    val cmd2dat_dma_addr = io.cmd2dat_dma_pd.bits(conf.NVDLA_MEM_ADDRESS_WIDTH-conf.AM_AW-1, 0)
    val cmd2dat_dma_size = io.cmd2dat_dma_pd.bits(conf.SDP_WR_CMD_DW-1, conf.NVDLA_MEM_ADDRESS_WIDTH - conf.AM_AW)
    val cmd2dat_dma_odd = io.cmd2dat_dma_pd.bits(conf.SDP_WR_CMD_DW)
    val cmd2dat_dma_cube_end = io.cmd2dat_dma_pd.bits(conf.SDP_WR_CMD_DW+1)
    ...
    when(io.cmd2dat_dma_pd.valid & io.cmd2dat_dma_pd.ready){
    ...
    val dat_pd_atom2 = Mux(beat_count(1,0) === 2.U, 
                        Cat(Fill(2*conf.AM_DW, false.B), io.dfifo_rd_pd(3).bits, io.dfifo_rd_pd(2).bits),
                        Cat(Fill(2*conf.AM_DW, false.B), io.dfifo_rd_pd(1).bits, io.dfifo_rd_pd(0).bits)
                        )
    ...
    val dma_wr_rdy = cfg_mode_quite || io.dma_wr_req_pd.ready
...
//=================================================
    ...
    for(i <- 0 to 3){
        when(io.op_load){
            ...
            when(io.dfifo_rd_pd(i).valid & io.dfifo_rd_pd(i).ready){
                dfifo_unequal(i) := dfifo_unequal(i) | (io.dfifo_rd_pd(i).bits.asUInt.orR)
            }
        }
    }
...
//==============
    ...
    io.intr_req_ptr.bits := io.reg2dp_interrupt_ptr
    io.intr_req_ptr.valid := layer_done
