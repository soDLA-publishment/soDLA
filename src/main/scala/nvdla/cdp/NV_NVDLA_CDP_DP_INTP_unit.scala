...
//(X1-X0)*frac
    ...
    val int_mul_s = RegInit(0.S(57.W))
    val interp_in0_pd_d1 = RegInit(0.U(17.W))
    val interp_in_shift_d1 = RegInit(0.U(6.W))
    when(int_in_load_d0){
        int_mul_s := int_sub.asSInt * interp_in_offset_d0.asSInt
        interp_in0_pd_d1 := interp_in0_pd_d0
        interp_in_shift_d1 := interp_in_shift_d0
    }
    int_mul := int_mul_s.asUInt

//>>16 proc for ((X1-X0)*frac) >>16

    val intp_in_shift_inv = !interp_in_shift_d1(4,0)
    val intp_in_shift_inv_inc = intp_in_shift_inv +& true.B
    val interp_in_shift_abs = Mux(interp_in_shift_d1(5), intp_in_shift_inv_inc, interp_in_shift_d1)
    val int_mul_shift_int = Wire(UInt(88.W))
    val int_mul_shift_frac = Wire(UInt(32.W))
    val int_mul_shift_int_and_frac = Mux(interp_in_shift_d1(5), 
                                        (Cat(Fill(31, int_mul(56)), int_mul(56,0), 0.U(32.W)) << interp_in_shift_abs),
                                        (Cat(Fill(31, int_mul(56)), int_mul(56,0), 0.U(32.W)) >> interp_in_shift_abs)
                                        )
    int_mul_shift_int := int_mul_shift_int_and_frac(118,32)
    int_mul_shift_frac := int_mul_shift_int_and_frac(31,0)

//rounding process for right shift
    val int_mul_for_Rshift = Reg(UInt(58.W))
    val int_mul_for_Rshift_s = Reg(SInt(58.W))

    when(int_mul_shift_int(56)){
        when(int_mul_shift_frac(31)){
            when(!(int_mul_shift_frac(30,0).asUInt.orR)){
                int_mul_for_Rshift := Cat(int_mul_shift_int(56), int_mul_shift_int(56,0))
            }.otherwise{
                int_mul_for_Rshift_s := (int_mul_shift_int(56,0)).asSInt + (Cat(0.U(56.W), true.B)).asSInt
                int_mul_for_Rshift := int_mul_for_Rshift_s.asUInt
            }
        }.otherwise{
            int_mul_for_Rshift := Cat(int_mul_shift_int(56), int_mul_shift_int(56,0))
        }
    }.otherwise{
        int_mul_for_Rshift_s := (int_mul_shift_int(56,0)).asSInt + (Cat(0.U(56.W),int_mul_shift_frac(31))).asSInt
        int_mul_for_Rshift := int_mul_for_Rshift_s.asUInt
    }
    val int_mul_rs = Mux(interp_in_shift_d1(5), 
                            int_mul_shift_int, 
                            Cat(Fill(30, int_mul_for_Rshift(57)), int_mul_for_Rshift(57,0)))
...
//Xo = X0+[(X1-X0)*frac>>16]
    ...
    val int_add_s = RegInit(0.S(89.W))
    when(int_in_load_d1){
        int_add_s := int_mul_rs.asSInt + (Cat(Fill(71, interp_in0_pd_d1(16)), interp_in0_pd_d1(16,0))).asSInt
    }
        int_add := int_add_s.asUInt

    val int_interp_out_pd = Mux(int_add(88), 
                                Mux((int_add(88,15)).asUInt.andR, 
                                    Cat(int_add(88),int_add(14,0)), 
                                    "h8000".asUInt(16.W)),
                                Mux((int_add(88,15)).asUInt.orR,
                                    "h7fff".asUInt(16.W),
                                    int_add(15,0))
                                )
    when(int_vld_d1){
