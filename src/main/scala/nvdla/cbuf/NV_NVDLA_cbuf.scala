        ...
        for(k <- 0 to conf.CBUF_RAM_PER_BANK-1){
            for(i <- 0 to conf.CBUF_WR_PORT_NUMBER-1){
                ...
                if(conf.CBUF_BANK_RAM_CASE==5){
                    bank_ram_wr_en_d0(j)(k)(i):=io.cdma2buf_wr_en(i)
                                                &&(io.cdma2buf_wr_addr(i)(conf.CBUF_BANK_SLICE_max,conf.CBUF_BANK_SLICE_min)===j.U) 
                                                &&(io.cdma2buf_wr_sel(j)(0)===k.U)&&(io.cdma2buf_wr_sel(i)(k%4)===true.B )
                }
            }
        }
    }
...
//generate sram write en (reduce port number)
    ...
    for(j <- 0 to conf.CBUF_BANK_NUMBER-1){
        for(k <- 0 to conf.CBUF_RAM_PER_BANK-1){          
            bank_ram_wr_en_d1(j)(k) := bank_ram_wr_en_d0(j)(k).map(_.reduce(_ | _)).reduce(_ | _) //OR-reduce from vector to bool                    
        }
    }
    ...
    for(i <- 0 to conf.CBUF_WR_PORT_NUMBER-){
        cdma2buf_wr_addr_d1(i) := io.cdma2buf_wr_addr(i)
        cdma2buf_wr_data_d1(i) := io.cdma2buf_wr_data(i)
    }
...
//generate bank write en (reduce ram per bank)
    ...
    for(i <- 0 to conf.CBUF_WR_PORT_NUMBER-1){
        for(j <- 0 to conf.CBUF_BANK_NUMBER-1){          
            bank_wr_en_d1(j)(i) := bank_wr_ram_en_d0(j)(i).map(_.reduce(_ | _)).reduce(_ | _) 
        }
    }
