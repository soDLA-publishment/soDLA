...
class NV_NVDLA_CSC_sg(implicit val conf: cscConfiguration) extends Module {
    ...
    val is_winograd = Wire(Bool())
    val data_out_atomic_w = Wire(UInt(22.W))
    val weight_width_cmp_w = Wire(UInt(5.W))
    val weight_height_cmp_w = Wire(UInt(5.W))

    if(conf.NVDLA_WINOGRAD_ENABLE){
        is_winograd := (io.reg2dp_conv_mode === 1.U)
        cur_mode := Cat(is_img, is_winograd, is_dc)
        data_out_atomic_w := Mux(is_img, io.reg2dp_dataout_width +& 1.U,
                             Mux(is_winograd, Cat("b0".asUInt(2.W), io.reg2dp_atomics(20, 2) + & 1.U),
                             io.reg2dp_atomics +& 1.U))
        weight_width_cmp_w := Mux((is_winograd | is_img),  "b0".asUInt(5.W), io.reg2dp_weight_width_ext)
        weight_height_cmp_w := Mux(is_img,  "b0".asUInt(5.W), io.reg2dp_weight_width_ext)
    }
    else{
    ...
    val lower_limit_w = if(conf.NVDLA_BATCH_ENABLE)
                        Mux(is_img, conf.CSC_IMG_STRIPE.U, conf.CSC_ATOMK_HEX.U)
                        else
                        Mux(is_img, conf.CSC_IMG_STRIPE.U, conf.CSC_ATOMK_HEX.U)
    ...
    val is_winograd_d1 = RegInit(false.B)
