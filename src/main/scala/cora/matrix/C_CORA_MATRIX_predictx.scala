...
// class C_CORA_MATRIX_predictx_fp(implicit conf: matrixConfiguration) extends Module {
...
//         val stat_out_exceptionFlags = Output(Vec(conf.KF_STAT, UInt(5.W)))  
...
//     val u_mac = Array.fill(conf.KF_STAT){Module(new MulAddRecFNPipe)}


//     umac.io.validin(i) := io.stat_actv_pvld(i) * io.tr_actv_pvld(i)
//     umac.io.op(i) := io.op
//     umac.io.
