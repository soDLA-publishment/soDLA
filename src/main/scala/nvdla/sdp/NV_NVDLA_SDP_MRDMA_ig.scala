...
class NV_NVDLA_SDP_MRDMA_ig(implicit conf: nvdlaConfig) extends Module {
   val io = IO(new Bundle {
        ...
        val ig2cq_pd = DecoupledIO(UInt(14.W))
...
withClock(io.nvdla_core_clk){
    ...
    io.ig2cq_pd.bits := Cat(ig2eg_cube_end, ig2eg_size)
    io.ig2cq_pd.valid := cmd_process & io.dma_rd_req_pd.ready

    //==============
    // DMA Req : PIPE
    //==============
    // VALID: clamp when when cq is not ready
    io.dma_rd_req_pd.valid := cmd_process & io.ig2cq_pd.ready
    io.dma_rd_req_pd.bits := Cat(dma_req_size, dma_req_addr)
    io.dma_rd_req_ram_type := io.reg2dp_src_ram_type

    // Accept
    cmd_accept := io.dma_rd_req_pd.ready & io.dma_rd_req_pd.ready

    //==============
    // PERF STATISTIC

    val u_histo_mrdma = Module{new NV_COUNTER_STAGE_histogram(32)}
    u_histo_mrdma.io.clk := io.nvdla_core_clk
    u_histo_mrdma.io.rd_stall_inc := io.dma_rd_req_pd.valid & !io.dma_rd_req_pd.ready
    u_histo_mrdma.io.rd_stall_dec := false.B
    u_histo_mrdma.io.rd_stall_clr := io.op_load
    u_histo_mrdma.io.rd_stall_cen := RegNext(io.reg2dp_perf_dma_en, false.B)
    io.dp2reg_mrdma_stall := u_histo_mrdma.io.cnt_cur
