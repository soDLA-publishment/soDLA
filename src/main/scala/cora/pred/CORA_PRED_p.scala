...
class CORA_PRED_p(implicit val conf: matrixConfiguration) extends Module {
    ...
    val io = IO(new Bundle {
        ...
        val tr_q_actv_data = Input(Vec(4, Vec(4, UInt(conf.KF_BPE.W))))
        val tr_q_actv_pvld = Input(Bool())
...
//             └──┴──┘       └──┴──┘ 
    ...
    //clock counter
    val pre_p_valid = io.tr_f_actv_pvld & io.tr_p_actv_pvld & io.tr_q_actv_pvld & tr_f_transpose_out_pvld
    val pre_p_valid_d1 = RegNext(pre_p_valid, false.B)
    val pre_p_st = ~pre_p_valid_d1 & pre_p_valid
