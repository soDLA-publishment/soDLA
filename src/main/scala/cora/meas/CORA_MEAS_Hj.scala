...
// class CORA_MEAS_hj(implicit val conf: matrixConfiguration) extends Module {

//     val io = IO(new Bundle {
//         //input
//         val reg2dp_roundingMode = Input(UInt(3.W))
//         val red2dp_detectTininess = Input(UInt(1.W))

//         val stat_actv_data = Input(Vec(4, UInt(conf.KF_BPE.W)))  
//         val stat_actv_pvld = Input(Bool())

//         //output
//         val hj_out_data = Output(Vec(4, Vec(4, UInt(conf.KF_BPE.W))))
//         val hj_out_pvld = Output(Bool())
...
//     //==========================================================
//     //clock counter
//     val meas_hj_valid = io.stat_actv_pvld
//     val meas_hj_valid_d1 = RegNext(meas_hj_valid, false.B)
//     val meas_hj_st = ~meas_hj_valid_d1 & meas_hj_valid
//     val meas_hj_done = Wire(Bool())

//     val clk_cnt = RegInit(0.U)
//     clk_cnt := Mux(meas_hj_st, 0.U,
//                Mux(meas_hj_done, 0.U,
//                clk_cnt + 1.U))
    
//     meas_hj_done := ( clk_cnt === (2*conf.V2V_MAC_LATENCY + conf.HARDFLOAT_MAC_LATENCY).U)
