...
class NV_NVDLA_CSC_SG_dat_fifo extends Module {
    ...
    val ram = Module(new NV_NVDLA_CSC_SG_dat_fifo_flopram_rwsa_4x33())
    ram.io.clk := io.clk
...
class NV_NVDLA_CSC_SG_dat_fifo_flopram_rwsa_4x33 extends Module {
    val io = IO(new Bundle {
        //clk
        val clk = Input(Clock())
        val clk_mgated = Input(Clock())
        
        val di = Input(UInt(33.W))
        val iwe = Input(Bool())
        val we = Input(Bool())
        val wa = Input(UInt(2.W))
        val ra = Input(UInt(3.W))
        val dout = Output(UInt(33.W))
...
withClock(io.clk){
    ...
    val di_d = Reg(UInt(33.W))
    when(io.iwe){
        di_d := io.di
    }

    withClock(io.clk_mgated){
        val ram_ff0 = Reg(UInt(33.W)) 
        val ram_ff1 = Reg(UInt(33.W))
        val ram_ff2 = Reg(UInt(33.W))
        val ram_ff3 = Reg(UInt(33.W))
        ...
        io.dout := MuxLookup(io.ra, "b0".asUInt(33.W), Seq(      
            0.U  -> ram_ff0,
            1.U  -> ram_ff1,
            2.U  -> ram_ff2,
            3.U  -> ram_ff3,
