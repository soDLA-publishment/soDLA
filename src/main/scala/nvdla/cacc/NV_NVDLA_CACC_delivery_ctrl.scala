...
// class NV_NVDLA_CACC_delivery_ctrl(implicit conf: nvdlaConfig) extends Module {
...
//         //dbuf
//         val dbuf_rd = new nvdla_rd_if(conf.CACC_ABUF_AWIDTH, conf.CACC_ABUF_WIDTH)
...
// dlv_pop := io.dbuf_rd.addr.valid & io.dbuf_rd_ready
// val dbuf_empty = ~(dlv_data_avl.orR)
// io.dbuf_rd.addr.valid := ~dbuf_empty
// io.dbuf_rd.addr.bits := dbuf_rd_addr_cnt

// when(dlv_pop){
//     dbuf_rd_addr_cnt := dbuf_rd_addr_cnt + 1.U
// }

// /////// generate dp2reg_done signal
// io.dp2reg_done := RegNext(io.dlv_valid & dlv_stripe_end & dlv_layer_end, false.B)
...
// dlv_end_tag0_vld := Mux(dlv_end_tag1_vld | dlv_end_set, true.B, Mux(dlv_end_clr, false.B, dlv_end_tag0_vld))
// dlv_end_tag1_vld := Mux(dlv_end_tag0_vld | dlv_end_set, true.B, Mux(dlv_end_clr, false.B, dlv_end_tag1_vld))

// when((dlv_end_set & ~dlv_end_tag0_vld) | (dlv_end_set & dlv_end_clr) |(dlv_end_clr & dlv_end_tag1_vld)){
//     dlv_end_tag0_addr := Mux(dlv_end_tag1_vld, dlv_end_tag1_addr, dbuf_wr_addr_pre)
// }
// when((dlv_end_set & dlv_end_tag0_vld & ~dlv_end_clr)){
