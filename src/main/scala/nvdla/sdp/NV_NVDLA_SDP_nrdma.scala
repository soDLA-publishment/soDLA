...
class NV_NVDLA_SDP_nrdma(implicit conf: sdpConfiguration) extends Module {
...
withClock(io.nvdla_core_clk){
    ...
    u_gate.io.rdma_disable := io.nrdma_disable
    u_gate.io.rdma_slcg_op_en := io.nrdma_slcg_op_en
    ...
    u_eg.io.sdp_rdma2dp_alu_ready := io.sdp_nrdma2dp_alu_ready
    u_eg.io.sdp_rdma2dp_mul_ready := io.sdp_nrdma2dp_mul_ready
    io.sdp_nrdma2dp_alu_pd := u_eg.io.sdp_rdma2dp_alu_pd
    io.sdp_nrdma2dp_alu_valid := u_eg.io.sdp_rdma2dp_alu_valid
    io.sdp_nrdma2dp_mul_pd := u_eg.io.sdp_rdma2dp_mul_pd
    io.sdp_nrdma2dp_mul_valid := u_eg.io.sdp_rdma2dp_mul_valid
    ...
    val u_lat_fifo = Module(new NV_NVDLA_SDP_fifo(conf.NVDLA_VMOD_SDP_NRDMA_LATENCY_FIFO_DEPTH, conf.NVDLA_DMA_RD_RSP))
...
object NV_NVDLA_SDP_nrdmaDriver extends App {
  implicit val conf: sdpConfiguration = new sdpConfiguration
  chisel3.Driver.execute(args, () => new NV_NVDLA_SDP_nrdma())
}
