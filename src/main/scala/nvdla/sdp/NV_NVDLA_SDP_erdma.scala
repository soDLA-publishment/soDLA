...
withClock(io.nvdla_clock.nvdla_core_clk){
    ...
    val u_cq = Module(new NV_NVDLA_fifo(depth = conf.NVDLA_VMOD_SDP_ERDMA_LATENCY_FIFO_DEPTH, width = 16, distant_wr_req = false, ram_type = 2))
    ...
    val u_lat_fifo = Module(new NV_NVDLA_fifo(depth = conf.NVDLA_VMOD_SDP_ERDMA_LATENCY_FIFO_DEPTH, width = conf.NVDLA_DMA_RD_RSP, distant_wr_req = false, ram_type = 2))
