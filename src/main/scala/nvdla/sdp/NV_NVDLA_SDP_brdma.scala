...
class NV_NVDLA_SDP_brdma(implicit conf: sdpConfiguration) extends Module {
...
val io = IO(new Bundle {
    ...
    val sdp_b2cvif_rd_req_pd = if(conf.NVDLA_SECONDARY_MEMIF_ENABLE) Some(Output(UInt(conf.NVDLA_DMA_RD_REQ.W))) else None
    ...
    val cvif2sdp_b_rd_rsp_pd = if(conf.NVDLA_SECONDARY_MEMIF_ENABLE) Some(Input(UInt(conf.NVDLA_DMA_RD_RSP.W))) else None
    ...
    val sdp_b2mcif_rd_req_pd = Output(UInt(conf.NVDLA_DMA_RD_REQ.W))
    val mcif2sdp_b_rd_rsp_valid  = Input(Bool())
    val mcif2sdp_b_rd_rsp_ready = Output(Bool())
    val mcif2sdp_b_rd_rsp_pd = Input(UInt(conf.NVDLA_DMA_RD_RSP.W))
    ...
    val sdp_brdma2dp_alu_pd = Output(UInt((conf.AM_DW2+1).W))

    val sdp_brdma2dp_mul_valid = Output(Bool()) 
    val sdp_brdma2dp_mul_ready = Input(Bool())  
    val sdp_brdma2dp_mul_pd = Output(UInt((conf.AM_DW2+1).W))
...
withClock(io.nvdla_core_clk){
    ...
    if(conf.NVDLA_SECONDARY_MEMIF_ENABLE){
        ...
        u_NV_NVDLA_SDP_RDMA_dmaif.io.sdp2cvif_rd_req_ready.get := io.sdp_b2cvif_rd_req_ready.get
        ...
        u_NV_NVDLA_SDP_RDMA_dmaif.io.cvif2sdp_rd_rsp_valid.get := io.cvif2sdp_b_rd_rsp_valid.get
        io.cvif2sdp_b_rd_rsp_ready.get := u_NV_NVDLA_SDP_RDMA_dmaif.io.cvif2sdp_rd_rsp_ready.get
