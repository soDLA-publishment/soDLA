...
class NV_NVDLA_SDP_HLS_x2_int(implicit conf: sdpConfiguration) extends Module {
   val io = IO(new Bundle {
        ...
        val chn_alu_op = Input(UInt(conf.BN_OP_DW.W))

        //chn_mul_op
        val chn_mul_op_pvld = Input(Bool())
        val chn_mul_op_prdy = Output(Bool())
        val chn_mul_op = Input(UInt(conf.BN_OP_DW.W))

        //chn_in
        val chn_in_pvld = Input(Bool())
        val chn_in_prdy = Output(Bool())
        val chn_data_in = Input(UInt(conf.BN_IN_DW.W))

        //chn_out
        val chn_out_pvld = Output(Bool())
        val chn_out_prdy = Input(Bool())
        val chn_data_out = Output(UInt(conf.BN_OUT_DW.W))
...
withClock(io.nvdla_core_clk){
   val alu_data_out_wire = Wire(Vec(conf.NVDLA_SDP_BN_THROUGHPUT, UInt(33.W)))
    val alu_out_prdy_wire = Wire(Vec(conf.NVDLA_SDP_BN_THROUGHPUT, Bool()))
    val alu_out_pvld_wire = Wire(Vec(conf.NVDLA_SDP_BN_THROUGHPUT, Bool()))
    val bypass_trt_out_wire = Wire(Vec(conf.NVDLA_SDP_BN_THROUGHPUT, Bool()))
    val chn_alu_op_wire = VecInit((0 to conf.NVDLA_SDP_BN_THROUGHPUT-1) 
                               map { i => io.chn_alu_op(16*i+15, 16*i)})
    val chn_alu_op_prdy_wire = Wire(Vec(conf.NVDLA_SDP_BN_THROUGHPUT, Bool()))
    val chn_data_in_wire = VecInit((0 to conf.NVDLA_SDP_BN_THROUGHPUT-1) 
                               map { i => io.chn_data_in(32*i+31, 32*i)})
    val chn_data_out_wire = Wire(Vec(conf.NVDLA_SDP_BN_THROUGHPUT, UInt(32.W)))
    val chn_in_prdy_wire = Wire(Vec(conf.NVDLA_SDP_BN_THROUGHPUT, Bool()))
    val chn_mul_op_wire = VecInit((0 to conf.NVDLA_SDP_BN_THROUGHPUT-1) 
                               map { i => io.chn_mul_op(16*i+15, 16*i)})
    val chn_mul_op_prdy_wire = Wire(Vec(conf.NVDLA_SDP_BN_THROUGHPUT, Bool()))
    val chn_out_pvld_wire = Wire(Vec(conf.NVDLA_SDP_BN_THROUGHPUT, Bool()))
    val mul_data_out_wire = Wire(Vec(conf.NVDLA_SDP_BN_THROUGHPUT, UInt(49.W)))
    val mul_out_prdy_wire = Wire(Vec(conf.NVDLA_SDP_BN_THROUGHPUT, Bool()))
    val mul_out_pvld_wire = Wire(Vec(conf.NVDLA_SDP_BN_THROUGHPUT, Bool()))
    val trt_data_out_wire = Wire(Vec(conf.NVDLA_SDP_BN_THROUGHPUT, UInt(32.W)))
    val trt_out_prdy_wire = Wire(Vec(conf.NVDLA_SDP_BN_THROUGHPUT, Bool()))
    val trt_out_pvld_wire = Wire(Vec(conf.NVDLA_SDP_BN_THROUGHPUT, Bool()))
    ...
    val u_sdp_x_alu = Array.fill(conf.NVDLA_SDP_BN_THROUGHPUT){Module(new NV_NVDLA_SDP_HLS_X_int_alu)}
    val u_sdp_x_mul = Array.fill(conf.NVDLA_SDP_BN_THROUGHPUT){Module(new NV_NVDLA_SDP_HLS_X_int_mul)}
    val u_sdp_x_trt = Array.fill(conf.NVDLA_SDP_BN_THROUGHPUT){Module(new NV_NVDLA_SDP_HLS_X_int_trt)}
    val u_sdp_x_relu = Array.fill(conf.NVDLA_SDP_BN_THROUGHPUT){Module(new NV_NVDLA_SDP_HLS_X_int_relu)}

    for(i <- 0 to conf.NVDLA_SDP_BN_THROUGHPUT-1){
...
object NV_NVDLA_SDP_HLS_x2_intDriver extends App {
  implicit val conf: sdpConfiguration = new sdpConfiguration
  chisel3.Driver.execute(args, () => new NV_NVDLA_SDP_HLS_x2_int)
}