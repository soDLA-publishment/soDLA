...
class NV_NVDLA_CDP_DP_sum(implicit val conf: nvdlaConfig) extends Module {
    val io = IO(new Bundle {
        ...
        val normalz_buf_data_pd = Flipped(DecoupledIO(UInt((conf.NVDLA_CDP_ICVTO_BWPE * (conf.NVDLA_CDP_THROUGHPUT + 8) + 17).W)))
        val sum2itp_pd = DecoupledIO(UInt((conf.NVDLA_CDP_THROUGHPUT * (conf.NVDLA_CDP_ICVTO_BWPE * 2 + 3)).W))
...
withClock(io.nvdla_core_clk){
    ...
    pipe_p1.io.vi := io.normalz_buf_data_pd.valid
    io.normalz_buf_data_pd.ready := pipe_p1.io.ro
    pipe_p1.io.di := io.normalz_buf_data_pd.bits
    ...
    //==========================================
    val buf2sum_int8 = VecInit((0 until conf.NVDLA_CDP_THROUGHPUT+8) map 
        {i => cdp_buf2sum_pd((i+1)*conf.NVDLA_CDP_ICVTO_BWPE-1, i*conf.NVDLA_CDP_ICVTO_BWPE)})
    ...
    for(i <- 0 until conf.NVDLA_CDP_THROUGHPUT+8){
        inv(i) := Mux(buf2sum_int8(i)(conf.NVDLA_CDP_ICVTO_BWPE-1), ~buf2sum_int8(i)(conf.NVDLA_CDP_ICVTO_BWPE-2,0), 0.U)
        int8_abs(i) := Mux(buf2sum_int8(i)(conf.NVDLA_CDP_ICVTO_BWPE-1), inv(i)(conf.NVDLA_CDP_ICVTO_BWPE-2,0) +& 1.U, buf2sum_int8(i))
    }
    ...
    val len7 = Wire(Bool())
    val len9 = Wire(Bool())
    val int8_sq = RegInit(VecInit((Seq.fill(conf.NVDLA_CDP_THROUGHPUT+8)(0.U((conf.NVDLA_CDP_ICVTO_BWPE*2-1).W)))))

    when(load_din){
        int8_sq(0) := Mux(len9, (int8_abs(0) * int8_abs(0)), 0.U)(conf.NVDLA_CDP_ICVTO_BWPE*2-2, 0)
        int8_sq(1) := Mux((len7 | len9), (int8_abs(1) * int8_abs(1)), 0.U)(conf.NVDLA_CDP_ICVTO_BWPE*2-2, 0)
        int8_sq(2) := Mux((len5 | len7 | len9), (int8_abs(2) * int8_abs(2)), 0.U)(conf.NVDLA_CDP_ICVTO_BWPE*2-2, 0)
        int8_sq(3) := (int8_abs(3) * int8_abs(3))(conf.NVDLA_CDP_ICVTO_BWPE*2-2, 0)

        for(i <- 4 until 4+conf.NVDLA_CDP_THROUGHPUT){
            int8_sq(i) := (int8_abs(i) * int8_abs(i))(conf.NVDLA_CDP_ICVTO_BWPE*2-2, 0)
        }

        int8_sq(conf.NVDLA_CDP_THROUGHPUT+4) := (int8_abs(conf.NVDLA_CDP_THROUGHPUT+4) * int8_abs(conf.NVDLA_CDP_THROUGHPUT+4))(conf.NVDLA_CDP_ICVTO_BWPE*2-2, 0)
        int8_sq(conf.NVDLA_CDP_THROUGHPUT+4+1) := Mux((len5 | len7 | len9), (int8_abs(conf.NVDLA_CDP_THROUGHPUT+4+1) * int8_abs(conf.NVDLA_CDP_THROUGHPUT+4+1)), 0.U)(conf.NVDLA_CDP_ICVTO_BWPE*2-2, 0)
        int8_sq(conf.NVDLA_CDP_THROUGHPUT+4+2) := Mux((len7 | len9), (int8_abs(conf.NVDLA_CDP_THROUGHPUT+4+2) * int8_abs(conf.NVDLA_CDP_THROUGHPUT+4+2)), 0.U)(conf.NVDLA_CDP_ICVTO_BWPE*2-2, 0)
        int8_sq(conf.NVDLA_CDP_THROUGHPUT+4+3) := Mux((len9), (int8_abs(conf.NVDLA_CDP_THROUGHPUT+4+3) * int8_abs(conf.NVDLA_CDP_THROUGHPUT+4+3)), 0.U)(conf.NVDLA_CDP_ICVTO_BWPE*2-2, 0)
    }
    ...
    //sum process
    //-----------
    len3 := (io.reg2dp_normalz_len === 0.U )
...
// NVDLA_CDP_THROUGHPUT = 8
    val sum_out_pd = int8_sum.asUInt
    ...
    io.sum2itp_pd.valid := pipe_p2.io.vo
    pipe_p2.io.ri := io.sum2itp_pd.ready
    io.sum2itp_pd.bits := pipe_p2.io.dout
