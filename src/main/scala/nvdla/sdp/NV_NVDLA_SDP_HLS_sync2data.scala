...
class NV_NVDLA_SDP_HLS_sync2data(DATA1_WIDTH:Int, DATA2_WIDTH:Int) extends Module {
   val io = IO(new Bundle {
        val chn1_en = Input(Bool())
        val chn1_in_pvld = Input(Bool())
        val chn1_in_prdy = Output(Bool())
        val data1_in = Input(SInt(DATA1_WIDTH.W))
        val data1_out = Output(SInt(DATA1_WIDTH.W))

        val chn2_en = Input(Bool())
        val chn2_in_pvld = Input(Bool())
        val chn2_in_prdy = Output(Bool())
        val data2_in = Input(SInt(DATA2_WIDTH.W))
        val data2_out = Output(SInt(DATA2_WIDTH.W))
    ...
    io.data1_out := Mux(io.chn1_en, io.data1_in, 0.asSInt)
    io.data2_out := Mux(io.chn2_en, io.data2_in, 0.asSInt)

}


object NV_NVDLA_SDP_HLS_sync2dataDriver extends App {
  chisel3.Driver.execute(args, () => new NV_NVDLA_SDP_HLS_sync2data(16, 32))
}