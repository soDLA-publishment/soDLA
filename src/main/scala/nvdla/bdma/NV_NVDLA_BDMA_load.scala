...
class NV_NVDLA_BDMA_load(implicit val conf: sdpConfiguration) extends Module {
    val io = IO(new Bundle {
        ...
        val bdma2cvif_rd_req_ready = if(conf.NVDLA_SECONDARY_MEMIF_ENABLE) Some(Input(Bool())) else None    /* data return handshake */
        val bdma2cvif_rd_req_pd = if(conf.NVDLA_SECONDARY_MEMIF_ENABLE) Some(Output(UInt(79.W))) else None
...
withClock(io.nvdla_core_clk){
    ...
    io.bdma2mcif_rd_req_valid := mc_int_rd_req_valid_d0;
    mc_int_rd_req_ready_d0 := io.bdma2mcif_rd_req_ready;
    io.bdma2mcif_rd_req_pd := mc_int_rd_req_pd_d0;
