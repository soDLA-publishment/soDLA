...
//     val dc2sbuf_p_wr = Vec(conf.ATMM_NUM, new nvdla_wr_if(8, conf.ATMM))
//     val dc2sbuf_p_rd = Vec(conf.ATMM_NUM, new nvdla_rd_if(8, conf.ATMM))
...
//     val req_ch_mode = Mux(is_packed_1x1, "h1".asUInt(3.W), min(conf.ATMM, conf.ATMC).U(3.W))
...
//     val req_addr_ch_base_add = io.reg2dp_surf_stride << log2Ceil(conf.ATMM)
...
//     io.dc2sbuf_p0_wr.data := dma_rsp_data_p0
...
//     }
//     io.dc2cvt_dat_wr.addr.bits := ShiftRegister(cbuf_wr_addr_0, 3, "b0".asUInt(17.W), cbuf_wr_en)
//     io.dc2cvt_dat_wr.data := ShiftRegister(io.dc2sbuf_p0_rd.data, 3, "b0".asUInt(conf.NVDLA_CDMA_DMAIF_BW.W), cbuf_wr_en)
//     io.dc2cvt_dat_wr.addr.valid := ShiftRegister(cbuf_wr_en, 3, false.B, true.B)
...
//     io.dc2status_dat_updt.valid := ShiftRegister(rsp_all_h_reg_en, 4, false.B, true.B)
//     io.dc2status_dat_updt.bits.entries := ShiftRegister(rsp_entry, 4, "b0".asUInt(15.W), rsp_all_h_reg_en)
//     io.dc2status_dat_updt.bits.slices := ShiftRegister(rsp_slice, 4, "b0".asUInt(14.W), rsp_all_h_reg_en)
