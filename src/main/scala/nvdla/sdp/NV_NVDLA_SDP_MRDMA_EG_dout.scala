...
class NV_NVDLA_SDP_MRDMA_EG_dout(implicit val conf: nvdlaConfig) extends Module{
   val io = IO(new Bundle {
        ...
        val cmd2dat_dma_pd = Flipped(DecoupledIO(UInt(15.W)))
        val pfifo_rd_pd = Vec(4, Flipped(DecoupledIO(UInt(conf.AM_DW.W))))
...
withClock(io.nvdla_core_clk){
    ...
    io.cmd2dat_dma_pd.ready := dat_accept & is_last_beat & fifo_vld & dat_rdy

    val cmd2dat_dma_size = io.cmd2dat_dma_pd.bits(13, 0)
    val cmd2dat_dma_cube_end  = io.cmd2dat_dma_pd.bits(14)
    val size_of_beat = (Fill(14, io.cmd2dat_dma_pd.valid)) & cmd2dat_dma_size
    val cmd_cube_end = io.cmd2dat_dma_pd.valid & cmd2dat_dma_cube_end
    ...
    for(i <- 0 to 3){
        ...
        pfifo_rd_sel_pvld(i) := io.pfifo_rd_pd(i).valid&pfifo_sel(i)
    }
    ...
    for(i <- 0 to 3){
        io.pfifo_rd_pd(i).ready := dat_rdy & pfifo_sel(i)
        pfifo_rd_data(i) := (Fill(conf.AM_DW, pfifo_sel(i))) & io.pfifo_rd_pd(i).bits
    }
    ...
    io.sdp_mrdma2cmux_pd.valid := pipe_p1.io.vo
    pipe_p1.io.ri := io.sdp_mrdma2cmux_pd.ready
    io.sdp_mrdma2cmux_pd.bits := pipe_p1.io.dout

    val sdp_mrdma2cmux_layer_end = io.sdp_mrdma2cmux_pd.bits(conf.DP_DIN_DW+1)
    io.eg_done := RegNext(sdp_mrdma2cmux_layer_end & io.sdp_mrdma2cmux_pd.valid & io.sdp_mrdma2cmux_pd.ready, false.B)
