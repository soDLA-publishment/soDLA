...
class NV_NVDLA_CSC_sg(implicit val conf: nvdlaConfig) extends Module {
    val io = IO(new Bundle {
        ...
        //cdma dat
        val cdma2sc_dat_updt = Flipped(ValidIO(new updt_entry_slices_if))
        ...
        //cdma wt
        val cdma2sc_wt_updt = Flipped(ValidIO(new updt_entry_kernels_if))    
...
withClock(io.nvdla_core_clk){
    ...
    when(layer_st){
        ...
        weight_r_add := ("h9".asUInt(6.W) << io.reg2dp_y_extension)(5, 3)
    ...
    when(pkg_adv){
        ...
        dat_pkg_stripe_length := cur_stripe
        dat_pkg_cur_sub_h := cur_r
        dat_pkg_block_end := is_last_block
        dat_pkg_channel_end := is_last_block & is_last_channel
        dat_pkg_group_end := is_last_block & is_last_channel & is_last_stripe & is_last_do_h
        dat_pkg_layer_end := is_last_block & is_last_channel & is_last_stripe & is_last_do_h & is_last_group
        dat_pkg_dat_release := ~io.reg2dp_skip_data_rls & is_last_block & is_last_channel & is_last_stripe & is_last_do_h & is_last_group
    }
