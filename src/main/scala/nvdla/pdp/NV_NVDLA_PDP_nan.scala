...
class NV_NVDLA_PDP_nan(implicit val conf: pdpConfiguration) extends Module {
    val io = IO(new Bundle {
        ...
        //rdma2dp
        val pdp_rdma2dp = Flipped(DecoupledIO(UInt(((conf.NVDLA_PDP_BWPE*conf.NVDLA_PDP_THROUGHPUT)+14).W)))
        //preproc
        val nan_preproc = DecoupledIO(UInt(((conf.NVDLA_PDP_BWPE*conf.NVDLA_PDP_THROUGHPUT)+14).W))
        //config  
